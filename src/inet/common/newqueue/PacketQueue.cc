...
void PacketQueue::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        if (*comparatorClass != '\0')
            ...
            packetComparatorFunction = new CPacketComparatorFunction((PacketComparatorFunction)function);
        }
        if (packetComparatorFunction != nullptr)
            queue.setup((CompareFunc)packetComparatorFunction->getComparatorFunction());
        // dropper function
        const char *dropperClass = par("dropperClass");
        const char *dropperFunction = par("dropperFunction");
        if (*dropperClass != '\0')
            packetDropperFunction = check_and_cast<IPacketDropperFunction *>(createOne(dropperClass));
        else if (*dropperFunction != '\0') {
            auto function = FunctionRegistry::globalRegistry.getFunction(dropperFunction);
            packetDropperFunction = new CPacketDropperFunction((PacketDropperFunction)function);
        }
